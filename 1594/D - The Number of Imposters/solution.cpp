...
int colour[200005];
bool flg;
void dfs(int idx,vi& v,vector<vpi>& adj){
    c[v[idx]]++;
    for(auto&[u,w] :adj[idx]){
        if(v[u]==-1){
            v[u]=v[idx]^w;
            dfs(u,v,adj);
        }
        else if(v[u]!=(v[idx]^w))flg=false;
    }
}
void test_cases(){
    ...
    f(i,0,m){
        ...
        string w;
        cin>>u>>v>>w;
        if(w=="imposter")adj[u].pb({v,1}),adj[v].pb({u,1});
        else adj[u].pb({v,0}),adj[v].pb({u,0});
    }
    int mx=0;
    flg=true;
    f(i,1,n+1){
