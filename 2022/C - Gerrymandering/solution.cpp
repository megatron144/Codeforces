...
// int dfs(int x,int y,vector<vi>& vis,vector<vi>& v,int& n){
//     if(!(x>=0 && x<2 && y<n && y>=0 && !vis[x][y] && !v[x][y]))return 0;
//     vis[x][y]=1;
//     return 1+dfs(x+1,y,vis,v,n)+dfs(x,y-1,vis,v,n)+dfs(x,y+1,vis,v,n)+dfs(x+1,y+1,vis,v,n)+dfs(x+1,y-1,vis,v,n);
// }
void test_cases(){
   // Your solution code here
    ...
    f(i,0,10)v[0].pb(0),v[1].pb(0);
    vector<vi> dp(n+10,vi(3,-1));
    dp[0][0]=0;
    f(i,0,n){
        f(j,0,3){
            ...
            switch(j){
                case 0:{
                    int vote_A=(v[0][i]+v[0][i+1]+v[0][i+2])/2 + (v[1][i]+v[1][i+1]+v[1][i+2])/2;
                    dp[i+3][0]=max(vote_A+val,dp[i+3][0]);
                    vote_A=(v[1][i]+v[0][i+1]+v[0][i])/2;
                    dp[i+1][1]=max(vote_A+val,dp[i+1][1]);
                    vote_A=(v[1][i]+v[1][i+1]+v[0][i])/2;
                    dp[i+1][2]=max(vote_A+val,dp[i+1][2]);
                    break;
                }
                case 1:{
                    int vote_A=(v[0][i+1]+v[0][i+2]+v[0][i+3])/2 + (v[1][i]+v[1][i+1]+v[1][i+2])/2;
                    dp[i+3][1]=max(vote_A+val,dp[i+3][1]);
                    vote_A=(v[1][i]+v[1][i+1]+v[0][i+1])/2;
                    dp[i+2][0]=max(vote_A+val,dp[i+2][0]);
                    break;
                }
                case 2:{
                    int vote_A=(v[1][i+1]+v[1][i+2]+v[1][i+3])/2 + (v[0][i]+v[0][i+1]+v[0][i+2])/2;
                    dp[i+3][2]=max(vote_A+val,dp[i+3][2]);
                    vote_A=(v[0][i]+v[0][i+1]+v[1][i+1])/2;
                    dp[i+2][0]=max(vote_A+val,dp[i+2][0]);
                    break;
                }
            }
