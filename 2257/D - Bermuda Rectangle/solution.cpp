...
void test_cases(){
    ...
    while(q--){
        int x,y;
        cin>>x>>y;
        int ix = upper_bound(all(v),x) - v.begin() - 1;
        int limit = s / y;
        int iy = upper_bound(all(v), limit) - v.begin() - 1;
        u128 cnt=0;
        int k=min(ix,iy);
        if(k>=0)cnt+=(u128)v[k]*y;
        if(ix>k){
            cnt+=pre[ix];
            if(k>=0)cnt-=pre[k];
        }
        if (ix+1<n){
            int prev=(ix>=0?v[ix]:0);
            int nxt=v[ix+1];
            if(x>prev)cnt+=(u128)(x-prev)*min(y,s/nxt);
        }
        print_u128(cnt);
        cout<<endl;
    }
