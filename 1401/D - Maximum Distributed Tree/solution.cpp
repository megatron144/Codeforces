...
void test_cases(){
    ...
    int cost=0;
    if(m<n){
        f(i,0,m)cost=add_mod(cost,prod_mod(val[i],v[i]));
        f(i,m,n-1)cost=add_mod(cost,v[i]);
    }
    else{
        cost=v[0];
        f(i,0,m-n+2)cost=prod_mod(cost,val[i]);
        f(i,1,n-1)cost=add_mod(cost,prod_mod(v[i],val[i+m-n+1]));
    }
    cout<<cost<<endl;
    v.clear();
}
