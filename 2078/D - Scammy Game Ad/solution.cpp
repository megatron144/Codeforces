...
void test_cases(){
    ...
    cin>>n;
    //0->+ 1->*
    vector<vi> val(n,vi (2,0)),op(n,vi (2,0));
    string op1,op2,val1,val2;
    f(i,0,n){
    ...
    vi post(n+1,0);
    for(int i=n-1;i>=0;i--){
        if((op[i][0] && op[i][1] && val[i][0]<val[i][1]) || (!op[i][0] && op[i][1]))
            post[i]=1;
        else if((op[i][0] && op[i][1] && val[i][0]>val[i][1]) || (!op[i][1] && op[i][0]))post[i]=0;
        else post[i]=post[i+1];
    }
    int val_left=1,val_right=1,cng=0;
    f(i,0,n){
        cng=0;
        if(op[i][0])cng+=(val[i][0]-1)*val_left;
        else cng+=val[i][0];
        if(op[i][1])cng+=(val[i][1]-1)*val_right;
        else cng+=val[i][1];
        if(post[i+1])val_right+=cng;
        else val_left+=cng;
    }
