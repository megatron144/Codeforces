...
void test_cases(){
    ...
    int n,m;
    cin>>n>>m;
    vector<vi> v(n,vi (m,0)),low(n,vi (m,0)),high(n,vi (m,0));
    f(i,0,n)f(j,0,m)cin>>v[i][j];
    if((m+n+1)&1){yn(0);return;}
    low[0][0]=high[0][0]=v[0][0];
    f(i,1,n)low[i][0]=high[i][0]=low[i-1][0]+v[i][0];
    f(i,1,m)low[0][i]=high[0][i]=low[0][i-1]+v[0][i];
    f(i,1,n){
        f(j,1,m){
            high[i][j]=max(high[i-1][j],high[i][j-1])+v[i][j];
            low[i][j]=min(low[i-1][j],low[i][j-1])+v[i][j];
        }
    }
    if(high[n-1][m-1]&1 || low[n-1][m-1]>0 || high[n-1][m-1]<0)yn(0);
    else yn(1);
}
int32_t main() {
