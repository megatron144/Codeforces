...
void make(string str,int idx){
    f(j,0,n)dp[idx][j+1]=dp[idx][j]+(s[j]!=str[j%3]);
}
void test_cases(){
    ...
    cin>>n>>q;
    cin>>s;
    dp.assign(6,vi (n+5,0));
    string x="abcab";
    f(i,0,3)make(x.substr(i,3),i);
    x="acbac";
    f(i,0,3)make(x.substr(i,3),i+3);
    while(q--){
        int l,r;
        cin>>l>>r;
        int ans=n;
        f(i,0,6)ans=min(ans,dp[i][r]-dp[i][l-1]);
        cout<<ans<<endl;
    }
}
int32_t main() {
