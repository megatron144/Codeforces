...
void solve(int pos,int d,int cnt,vi sP,vi& divs,vi& cng) {
    if(pos==4){
        divs.pb(d);
        cng.pb(cnt%2?-1:1);
        return;
    }
    solve(pos+1,d,cnt,sP,divs,cng);
    solve(pos+1,d*sP[pos],cnt+1,sP,divs,cng);
}
int help(int n,vi& divs,vi& cng){
    if(n<=0)return 0;
    int cnt=0;
    f(i,0,divs.size())
        cnt+=cng[i]*(n/divs[i]);
...
void test_cases() {
    ...
    cin>>l>>r;
    vi divs,cng;
    solve(0,1,0,{2,3,5,7},divs,cng);
    cout<<help(r,divs,cng)-help(l-1,divs,cng)<<endl;
}
int32_t main() {
