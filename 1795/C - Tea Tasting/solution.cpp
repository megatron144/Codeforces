...
void test_cases() {
    ...
    int n;
    cin>>n;
    vi a(n),b(n),pre(n+1,0),sweep(n+1,0);
    take(a);
    take(b);
    vpi v;
    f(i,0,n){
    ...
    f(i,0,n){
        ...
        while(l<=h){
            mid=l+((h-l)>>1);
            if((pre[mid]-pre[i])>a[i])h=mid-1;
            else l=mid+1;
        }
        sweep[h]++;
        v[h].first+=(a[i]-pre[h]+pre[i]);
        // while((pre[j]-pre[i])>a[i]){
        //     v[j-1].first-=v[j-1].second;
        //     j--;
        // }
        // // if(a[i]>=b[i])
        // v[j].first+=(a[i]-pre[j]+pre[i]);
        // //else 
    }
    int cnt=0;
    f(i,0,n){
        cnt+=sweep[i];
        cout<<(v[i].first-(cnt*v[i].second))<<" ";
    }
    cout<<endl;
    return;
}
