...
void test_cases() {
    ...
    int L=(l-k)>>i,R=(r-k)>>i;
    if((l-k)%(1<<i))L++;
    int uninteresting_cnt=(R-L+1);
    int tot_xor=range_xor(r)^range_xor(l-1);
    int skip_xor=range_xor(R)^range_xor(L-1);
    skip_xor=skip_xor<<i;
    cout<<(tot_xor^skip_xor^((uninteresting_cnt&1)?k:0))<<endl;
}
int32_t main() {
