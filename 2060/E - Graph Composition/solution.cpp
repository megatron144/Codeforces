...
void dfs(int i,vi& vis,vector<vi>& G,vi& mark,int val){
    if(vis[i])return;
    vis[i]=1;
    mark[i]=val;
    for(auto x: G[i]){
        if(vis[x])continue;
        dfs(x,vis,G,mark,val);
    }
}
void test_cases() {
    ...
    f(i,0,n){
        if(vis[i])continue;
        dfs(i,vis,G,mark,gcnt);
        gcnt++;
    }
    f(i,0,n){
        for(auto x: F[i]){
            if(mark[i]==mark[x])FN[i].pb(x);
            else cnt++;
        }
    }
    cnt=cnt>>1;
