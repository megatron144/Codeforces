...
void test_cases(){
    ...
    vi v,freq;
    freq.pb(1);
    int sz=mp.size(),cnt=0,prod=1;
    if(sz<m)goto there;
    freq.assign(sz,0);
    for(auto& x: mp)v.pb(x.first);
    sort(all(v));
    f(i,0,sz)freq[i]=mp[v[i]];
    f(i,0,m)prod=prod_mod(prod,freq[i]);
    f(i,0,sz-m+1){
        if(v[i+m-1]==(v[i]+m-1))cnt=add_mod(cnt,prod);
        if(i+m<sz){
            int inv=power(freq[i],MOD-2,MOD);
            prod=prod_mod(prod,inv);
            prod=prod_mod(prod,freq[i+m]);
        }
    }
    there:
    cout<<cnt<<endl;
