...
int pre[100006][30][2],suf[100006][30][2];
void test_cases(){
    ...
    vi v(n);
    take(v);
    f(i,0,30)suf[n][i][0]=suf[n][i][1]=0;
    //vector<vector<vi>> pre(n+1,vector<vi> (30,vi(2,0))),suf(n+1,vector<vi> (30,vi(2,0)));
    f(bit,0,30){
        f(i,0,n){
            int curr=(v[i]>>bit)&1;
            pre[i+1][bit][0]=pre[i][bit][curr]+(curr==0);
            pre[i+1][bit][1]=pre[i][bit][curr^1]+(curr==1);
        }
        for(int i=n-1;i>=0;i--){
            int curr=(v[i]>>bit)&1;
            suf[i][bit][0]=suf[i+1][bit][curr]+(curr==0);
            suf[i][bit][1]=suf[i+1][bit][curr^1]+(curr==1);
        }
    }
    long long cnt=0;
    f(i,0,n){
        int idx=31-__builtin_clz(v[i]);
        cnt+=1LL*pre[i][idx][1]*suf[i+1][idx][0]+1LL*pre[i][idx][0]*suf[i+1][idx][1]+(long long)suf[i+1][idx][1]+(long long)pre[i][idx][1];
    }
    cout<<cnt<<endl;
}
int32_t main() {
