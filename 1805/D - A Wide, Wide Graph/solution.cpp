...
vector<vi> adj;
vi dis,max_dis;
void dfs(int node,int par,int len,bool flg){
    if(flg)dis[node]=len;
    else max_dis[node]=len;
    for(auto x: adj[node])if(x^par)dfs(x,node,len+1,flg);
}
void test_cases(){
    ...
    dfs(1,0,0,true);
    int maxi=max_element(all(dis))-dis.begin();
    dfs(maxi,0,0,true);
    maxi=max_element(all(dis))-dis.begin();
    dfs(maxi,0,0,false);
    f(i,1,n+1)max_dis[i]=max(max_dis[i],dis[i]);
    sort(all(max_dis));
    int cnt=0;
    f(i,1,n+1){
        while(cnt<n && max_dis[cnt]<i)cnt++;
        cout<<cnt<<" ";
    }
    cout<<endl;
}
