...
void test_cases(){
    ...
    f(i,0,n){
        cin>>s;
        f(j,0,m){
            if(s[j]=='1')mat[i][j].first++,tar+=v[i][j];
            else mat[i][j].second++,tar-=v[i][j];
        }
    }
    if(tar==0){yn(1);return;}
    tar=abs(tar);
    //0->n-sz+1
    f(i,0,n-sz+1){
        f(j,0,m){
            f(k,i+1,i+sz){
                mat[i][j].first+=mat[k][j].first;
                mat[i][j].second+=mat[k][j].second;
            }
        }
    }
    int g=0;
