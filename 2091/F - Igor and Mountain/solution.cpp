...
void test_cases(){
    ...
    cin>>n>>m>>d;
    char c;
    vi pref(m+1,0),curr(m);
    int d_prev=(int)sqrt(d*d-1);
    vector<vi> grid(n,vi (m,0));
    f(i,0,n)f(j,0,m)cin>>c,grid[i][j]=(c=='X');
    //f(i,0,n){print(grid[i]);}
    //row 0
    curr=grid[0];
    f(i,1,m+1)pref[i]=add_mod(pref[i-1],curr[i-1],MOD);
    //print(pref);
    curr.resize(m,0);
    f(i,0,m){
        if(grid[0][i]){
            int r=min(i+d+1,m),l=max(0LL,i-d);
            curr[i]=pref[r]-pref[l];
        }
    }
    //print(curr);
    f(i,1,n){
        f(j,1,m+1)pref[j]=add_mod(pref[j-1],curr[j-1],MOD);
        //print(pref);
        curr.assign(m,0);
        f(j,0,m)if(grid[i][j])curr[j]=pref[min(j+d_prev+1,m)]-pref[max(0LL,j-d_prev)];
        //print(curr);
        f(j,1,m+1)pref[j]=add_mod(pref[j-1],curr[j-1],MOD);
        //print(pref);
        f(j,0,m)if(grid[i][j])curr[j]=pref[min(j+d+1,m)]-pref[max(0LL,j-d)];
        //print(curr);
    }
    int cnt=0;
    //print(curr);
    for(auto x: curr)cnt=add_mod(x,cnt,MOD);
    cout<<cnt<<endl;
}
int32_t main() {
