...
void test_cases(){
    ...
    take(a);
    take(b);
    vi neig_l(n+1),neig_r(n+1),vis(n+1,0);
    for(auto x: b)vis[x]=1;
    vis[0]=1;
    f(i,1,n)neig_r[a[i-1]]=a[i],neig_l[a[i]]=a[i-1];
    int cnt=1;
    for(auto x: b){
        int l=vis[neig_l[x]],r=vis[neig_r[x]];
        if(l && r){
            cout<<0<<endl;
            return;
        }
        cnt=prod_mod(cnt,2-l-r);
        neig_l[neig_r[x]]=neig_l[x];
        neig_r[neig_l[x]]=neig_r[x];
    }
    cout<<cnt<<endl;
}
int32_t main() {
