...
void test_cases(){
	...
	f(i,0,n)last[a[i]]=i;
	int ans=0,l=0;
	vi prev,was(n+1),curr(n+1),del(n+1);
	int mw=1,mc=1,md=1;
	while(l<n){
		vi tmp;
		int sz=prev.size(),mini=n;
		f(i,l,n){
			int v=a[i];
			if(curr[v]!=mc){
			    curr[v]=mc;
			    tmp.pb(v);
			    mini=min(mini,last[v]);
			}
			if(was[v]==mw && del[v]!=md){
			    del[v]=md;
			    sz--;
			}
			if(sz==0 && ((i==n-1)||(mini>i))){
				ans++;
				//print(tmp);
				prev=tmp;
				mw++;
				for(auto x:prev)was[x]=mw;
				l=i+1;
				break;
			}
		}
		if((l<n && prev.empty()) || (l<n && tmp.empty()))break;
		mc++;
		md++;
	}
	cout<<ans<<endl;
