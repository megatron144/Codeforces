...
int check(int u,vector<vi>& v,vector<bool>& vis){
    vis[u]=true;
    for(auto& x: v[u])
        if(!vis[x])return check(x,v,vis)+1;
    return 1;
}
void test_cases(){
    ...
    f(i,0,n){
        ...
        v[u].pb(vv);
        v[vv].pb(u);
        if(u==vv || v[vv].size()>2 || v[u].size()>2)flg=true;
    }
    if(flg){yn(0);return;}
    vector<bool> vis(n+1,false);
    f(i,1,n+1)if(!vis[i])if(check(i,v,vis)&1){yn(0);return;}
    yn(1);
}
int32_t main() {
