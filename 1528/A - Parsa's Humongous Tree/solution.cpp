...
void dfs(int node,int p){
    for(auto x: adj[node]){
        if(x==p)continue;
        dfs(x,node);
        dp[node][0]+=max(abs(v[node].first-v[x].second)+dp[x][1],abs(v[node].first-v[x].first)+dp[x][0]);
        dp[node][1]+=max(abs(v[node].second-v[x].second)+dp[x][1],abs(v[node].second-v[x].first)+dp[x][0]);   
    }
}
// 7 8 62
void test_cases(){
