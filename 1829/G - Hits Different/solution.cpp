...
//vector<vector<u128>> v(2030,vector<u128> (2030,(u128)0));
vector<u128> ans(MAXN);
vector<vector<u128>> v(1505,vector<u128> (1505));
void test_cases(){
    // Your solution code here
    int n;cin>>n;
    // u128 x=(u128)n*n;
    // int lo=1,hi=2000000,row=hi;
    // while(lo<=hi){
    //     int mid=(lo+hi)>>1;
    //     u128 tri=(u128)mid*(mid+1)/2;
    //     if(tri>=x){row=mid;hi=mid-1;}
    //     else lo=mid+1;
    // }
    // u128 prev=(u128)row*(row-1)/2;
    // int pos=(int)((long long)(x-prev));
    // u128 ans=0;
    // for(int i=row;i>=1;--i){
    //     int d=row-i;
    //     int l=pos-d;
    ...
    //     if(r>i)r=i;
    //     if(l>r)continue;
    //     u128 start=(u128)i*(i-1)/2+1;
    //     u128 first=start+(u128)(l-1);
    //     u128 last=start+(u128)(r-1);
    //     u128 cnt=(u128)(r-l+1);
...
int32_t main() {
    ...
    for(int i=1;i<1500;i++){
        for(int j=i-1;j>=1;j--){
            v[j][i-j]=(u128)v[j-1][i-j]+v[j][i-j-1]-v[j-1][i-j-1]+curr*curr;
            ans[curr]=(u128)v[j][i-j];
            curr++;
        }
    }
    int t=1;
