...
vi fen_tree;
void update_tree(int i,int v,int n){
    for(;i<=(2*n+100);i+=i&-i)fen_tree[i]=max(fen_tree[i],v);
}
int max_pref(int i){
    int maxi=0;
    for(;i>0;i-=i&-i)maxi=max(maxi,fen_tree[i]);
    return maxi;
}
void test_cases(){
    ...
    take(a);
    take(b);
    fen_tree.assign(2*n+101,0);
    vi st;
    int ans=0;
    f(i,0,n){
        while(!st.empty() && a[st.back()]<a[i])st.pop_back();
        int l=st.empty()?-1:st.back(),idx=2*n-b[i]+1;
        int max_pos=max_pref(idx);
        int cnt=(a[i]==b[i])?(i-l):0;
        cnt+=min(l+1,max_pos);
        ans+=cnt*(n-i);
        update_tree(2*n-a[i]+1,i+1,n);
        st.pb(i);
    }
    fen_tree.clear();
    cout<<ans<<endl;
