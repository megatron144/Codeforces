...
void test_cases(){
    ...
    f(i,1,n+1){
        leaves++;
        int sz=bridges[i].size();
        if(sz==1)continue;
        leaves--;
        int cnt=0;
        for(auto& x: bridges[i])cnt+=(bridges[x].size()==1);
        if(cnt>=sz-2)ways=prod_mod(ways,fact[cnt],MOD);
        else{cout<<0<<endl;return;}
    }
    if(leaves<n-1)cout<<prod_mod(ways,2,MOD)<<endl;
    else cout<<ways<<endl;
}
int32_t main() {
