...
bool dfs(int p,vector<vi>& adj,vi& vis){
    if(vis[p])return false;
    vis[p]=1;
    bool flg=true;
    for(auto& v: adj[p])flg&=dfs(v,adj,vis);
    return flg;
}
void test_cases(){
    ...
    umpi mp;
    int x;
    vector<vi> adj(n+1),v(k,vi (n));
    f(i,0,k)f(j,0,n)cin>>v[i][j];
    f(i,0,k){
        f(j,1,n-1){
            int p=v[i][j],node=v[i][j+1];
            int key=((p*3557)+(node*9137)+1007*p*node)%MOD;
