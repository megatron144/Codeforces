...
// bool check(int i,int& n,vi& v,vi& dp){
//     if(i==n)return true;
//     if(dp[i]!=-1)return dp[i];
//     bool flg=false;
//     int L=v[i];
//     if(i+1+L<=n){
//         flg |= check(i+1+L,n,v,dp);
//     }
//     if(i-L-1==-1 || (i-L-1>=0 && dp[i-L-1]==1)){
//         flg |= check(i+1,n,v,dp);
//     }
//     return dp[i]=flg;
// }
// YES
...
void test_cases(){
    ...
    dp[0] = true;
    f(i,0,n){
        if(i+1+v[i]<=n && dp[i])dp[i+1+v[i]]=true;
        if(i-v[i]>=0 && dp[i-v[i]])dp[i+1]=true;
    }
    yn(dp[n]);
}
int32_t main() {
