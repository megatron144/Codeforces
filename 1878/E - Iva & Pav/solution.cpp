...
bool safe(int mid,vector<vi>&dp,int k,int l){
	int sum=0;
	f(i,0,32){
		int cnt=dp[mid][i]-(l>0?dp[l-1][i]:0);
		if(cnt==mid-l+1)sum|=1<<i;
	}
	return sum>=k;
}
void test_cases(){
	...
	vi v(n);
	take(v);
	vector<vi> dp(n,vi(32,0));
	f(i,0,n)
		f(j,0,32)
			dp[i][j]=(i>0?dp[i-1][j]:0)+((v[i]>>j)&1);
	int q;
	cin>>q;
	while(q--){
		int l,k;
