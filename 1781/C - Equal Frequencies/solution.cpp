...
void test_cases(){
    ...
    string s;
    cin>>s;
    vector<vi> freq_pos(26);
    f(i,0,n) freq_pos[s[i]-'a'].pb(i);
    vi order(26);
    iota(order.begin(), order.end(), 0);
    sort(all(order), [&](int a,int b){
        return freq_pos[a].size() > freq_pos[b].size();
    });
    int mx=0,k=-1;
    f(cnt,1,27){
        if(n%cnt)continue;
        int curr=0;
        f(i,0,cnt)curr+=min((int)freq_pos[order[i]].size(),n/cnt);
        if(curr>mx)mx=curr,k=cnt;
    }
    cout<<n-mx<<endl;
    string str(n,' ');
    vector<char> chars;
    f(i,0,k){
        int ch=order[i];
        f(j,0,n/k){
            if(j<freq_pos[ch].size())str[freq_pos[ch][j]]=(char)('a'+ch);
            else chars.pb((char)('a'+ch));
        }
    }
    f(i,0,n){
        if(str[i]==' '){
