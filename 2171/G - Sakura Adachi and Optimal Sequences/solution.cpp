...
void test_cases(){
    ...
    f(i,0,n){
        int t1=a[i],t2=b[i];
        int cnt=1;
        while(t1<t2)t1<<=1,cnt++;
        mx=min(mx,cnt);
    }
    int mn=1e12,mul=0;
    for(int mid=mx;mid>=0;mid--){
        int cnt=mid;
        f(i,0,n){
            int den=(1<<mid),curr=a[i];
            curr<<=mid;
            if(curr>b[i])goto there;
            curr=a[i];
            while(den){
                cnt+=(b[i]/den)-curr;
                curr=(b[i]/den);
        ...
        if(mn>=cnt)mn=cnt,mul=mid;
        there:
    }
    int ways=1;
    for(int steps=mul;steps>=0;steps--){
        int num=0,den=1;
        f(i,0,n){
            int cng=(b[i]/(1<<steps))-a[i];
            den=prod_mod(den,fact[cng]);
            num+=cng;
            a[i]=(b[i]/(1<<steps))<<1;
        }
        if(num>=MOD){ways=0;break;}
        ways=prod_mod(ways,divide_mod(fact[num],den));
    }
    //cout<<mul<<" ";
    cout<<mn<<" "<<(ways%MOD)<<endl;
}
// 17 827116
// 3 1
// 12 288
