...
void test_cases(){
    ...
    vi v(n);
    take(v);
    if(v[0]==n || v[n-1]==1){yn(0);return;}
    vi pre(n+1,n);
    vpi suf(n+1,{0,n});
    f(i,1,n+1)pre[i]=min(pre[i-1],v[i-1]);
    for(int i=n-1;i>=0;i--)suf[i]=max(suf[i+1],{v[i],i});
    f(i,1,n)if(pre[i]>suf[i].first){yn(0);return;}
    yn(1);
    int l=0;
    while(l<n){
        int r=suf[l].second;
        for(int i=l;i<r;i++)cout<<v[i]<<" "<<v[r]<<endl;
        if(l>0)cout<<pre[l]<<" "<<v[r]<<endl;
        l=r+1;
    }
}
int32_t main() {
