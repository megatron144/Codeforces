...
void test_cases(){
    ...
    sort(all(query),[blocks](vector<int>& a,vector<int>& b){
        int a0=a[0]/blocks,b0=b[0]/blocks;
        if(a0!=b0)return a0<b0;
        return (a0&1)?a[1]>b[1]:a[1]<b[1];
    });
    vi ans(q,1),freq(*max_element(all(v))+1,0),freq_cnt(n+2,0);
    freq_cnt[0]=n;
    int mx=0;
    auto addE=[&](int idx) {
        int val=v[idx],curr=freq[val];
        freq_cnt[curr]--;
        freq[val]++;
        freq_cnt[curr+1]++;
        if(curr+1>mx)mx=curr+1;
    };
    auto removeE=[&](int idx) {
        int val=v[idx],curr=freq[val];
        freq_cnt[curr]--;
        freq[val]--;
        freq_cnt[curr-1]++;
        if(curr==mx && !freq_cnt[curr])mx--;
    };
    int cl=0,cr=-1;
    for(auto x: query){
        ...
        while(cl>l)addE(--cl);
        while(cr<r)addE(++cr);
        while(cl<l)removeE(cl++);
        while(cr>r)removeE(cr--);
        int len=r-l+1;
        int lim=len+1;
        lim>>=1;
        if(mx>lim)ans[idx]=2*mx-len;
    }
    print(ans);
}
int32_t main() {
