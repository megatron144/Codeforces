...
void test_cases(){
    ...
    f(i,0,k)if(!L[i].empty())pq.push({L[i][0].first,L[i][0].second,i,0});
    while(!pq.empty()){
        auto [mn,s,i,j]=pq.top();
        pq.pop();
        if(x+mn<0)break;
        x+=s;
        if(j+1<L[i].size())pq.push({L[i][j+1].first,L[i][j+1].second,i,j+1});
    }
    cout<<x<<endl;
}
int32_t main() {
