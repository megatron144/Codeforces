...
int n;
int states[3];
int dfs(int idx,int phase,int len){
    if(idx==n)return (len==3)?1:0;
    if(dp[idx][phase][len]!=-1)return dp[idx][phase][len];
    int total=0;
    total=add_mod(total,dfs(idx+1,phase,len),MOD);
    if(v[idx]==states[phase])
        total=add_mod(total,dfs(idx+1,(phase+1)%3,min(3LL,len+1)),MOD);
    return dp[idx][phase][len]=total;
}
void test_cases(){
    ...
    v.resize(n);
    int even=0;
    for(auto& x: v)cin>>x,x%=2,even+=(x==0);
    dp.resize(n+1,vector<vi> (3,vi (4,-1)));
    int all_even=0;
    if(even>=3){
        all_even=power(2,even,MOD);
        all_even-=even+1;
        even=even*(even-1);
        even>>=1;
        all_even-=even;
    ...
    f(x,0,2){
        f(y,0,2){
            ...
            states[1]=y;
            states[2]=(x^y);
            dp.assign(n+1,vector<vi> (3,vi (4,-1)));
            cnt=add_mod(cnt,dfs(0,0,0),MOD);
        }
    }
    cout<<add_mod(all_even,cnt,MOD)<<endl;
}
int32_t main() {
