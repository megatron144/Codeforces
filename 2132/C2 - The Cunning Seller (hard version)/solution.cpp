...
void test_cases() {
    ...
    int curr=s,sz=v.size();
    v.resize(sz+10,0);
    for(int i=(int)v.size()-1;i>=1 && curr<k;i--){
        if(v[i]==0)continue;
        k--;
        if(k<curr)break;
        k++;
        int take=min(v[i],(k-curr)>>1);
        v[i]-=take;
        v[i-1]+=3*take;
        curr+=2*take;
    }
    vector<ull> p(v.size()+3);p[0]=1;
    f(i,1,(int)p.size())p[i]=p[i-1]*3ULL;
    u128 cost=0;
    f(i,0,(int)v.size()){
        if(v[i]==0)continue;
        if(i==0)cost+=(u128)v[i]*3ULL;
        else cost+=(u128)v[i]*((u128)p[i+1]+(u128)i*p[i-1]);
    }
    print_u128(cost);
    cout<<endl;
}
