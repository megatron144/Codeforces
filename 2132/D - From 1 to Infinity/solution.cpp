...
u128 help(u128 n){
    ...
    int d=0;
    while(p[d+1]<=n)d++;
    u128 curr=p[d],msd=n/curr,r=n%curr;
    return msd*sum(d)+(msd*(msd-1)/2)*curr+msd*(r+1)+help(r);
}
// 15
// 46
// 48
...
void test_cases(){
    int K;
    cin>>K;
    u128 k=K,ans=0,base=1;
    int d=1;
    while(1){
        u128 len=(u128)d*9*base;
        if(k>=len){
            u128 end=p[d]-1;
            ans+=help(end)-help(base-1);
            k-=len;
            d++;
            base*=10;
        }else{
            ...
            int rem=k%d;
            if(full>0){
                u128 last=base+full-1;
                ans+=help(last)-help(base-1);
            }
            if(rem>0){
                int nxt=(int)(base+full);
