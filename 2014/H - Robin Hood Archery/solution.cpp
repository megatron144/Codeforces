...
// int binary_search(int target,vi& v){
//     int mid,idx=-1,l=0,h=v.size()-1;
//     while(l<=h){
//         mid=(h+l)>>1;
//         if(v[mid]<=target)idx=mid,l=mid+1;
//         else h=mid-1;
//     }
//     return idx;
// }
void test_cases() {
    ...
    vi v(n);
    take(v);
    for(auto& x: v)x=(5377*x+401*power(x,3,MOD))%MOD;
    vi pre_xor(n+1,0);
    f(i,0,n)pre_xor[i+1]=pre_xor[i]^v[i];
    int l,r;
    while(q--){
        cin>>l>>r;
        yn(!(pre_xor[r]^pre_xor[l-1]));
    }
}
int32_t main() {
