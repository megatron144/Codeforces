...
void test_cases(){
    ...
    int sides=0;
    for(auto x: mp){
        if(x.second%2)uniq.pb(x.first);
        int cnt=x.second/2*2;
        sum+=(ull)cnt*x.first;
        sides+=cnt;
    }
    ull p=(sides>=2)?sum:0;
    sort(allR(uniq));
    int idx=0,sz=uniq.size();
    while(idx<sz){
        if(sum>(ull)uniq[idx]){p+=(ull)uniq[idx],sides++;break;}
        idx++;
    }
    idx=0;
    while(idx<sz-1){
        if((sum+(ull)uniq[idx+1])>(ull)uniq[idx]){
            p=max(p,sum+(ull)uniq[idx+1]+(ull)uniq[idx]);
            sides++;
            break;
        }
        idx++;
