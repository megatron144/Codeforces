...
void test_cases(){
    ...
    cin>>n;
    char c;
    vector<vi> v(2,vi(n+5,0));
    int cnt=0,multiple=0;;
    f(i,1,n+1){
        cin>>c;
        if(c=='.')v[0][i]=1,cnt++;
    }
    ...
    f(i,1,n+1){
        if(v[0][i] && v[1][i]){
            if(v[0][i+1] && v[1][i+1])multiple=1;
            blocks++;
            v[0][i]=v[1][i]=0;
        }
        if(v[0][i]){
            if(v[0][i+1])v[0][i+1]=0,blocks++;
            else{
                cout<<"None"<<endl;
                return;
            }
    ...
    if(blocks==cnt)cout<<(multiple?"Multiple":"Unique")<<endl;
    else cout<<"None"<<endl;
    // vector<vi> dp(n+8,vi(3,0));
    // f(i,0,n+3){
    //     f(j,0,3){
    //         int val=dp[i][j];
    //         if(j==0){
    //             //+1
    //             //all 4
    //             if(v[0][i+1]&v[1][i+1]&v[0][i+2]&v[1][i+2])dp[i+2][0]=max(dp[i+2][0],2+val),multiple=1;
    //             if(v[0][i+1]&v[1][i+2])dp[i+2][2]=max(dp[i+2][2],1+val);
    //             if(v[0][i+1]&v[0][i+2])dp[i+2][1]=max(dp[i+2][1],1+val);
    //             if(v[1][i+1]&v[0][i+1])dp[i+1][0]=max(dp[i+1][0],1+val);
    //         }
    //         else if(j==1){
    //             if(v[0][i+3]&v[1][i+3]&v[0][i+2]&v[1][i+2])dp[i+3][1]=max(dp[i+3][1],2+val),multiple=1;
    //             if(v[1][i+1]&v[1][i+2])dp[i+2][2]=max(dp[i+2][2],1+val);
    //             if(v[1][i+2]&v[0][i+2])dp[i+2][1]=max(dp[i+2][1],1+val);
    //         }
    //         else{
    //             if(v[0][i+1]&v[1][i+1]&v[0][i+2]&v[1][i+2])dp[i+2][2]=max(dp[i+2][2],2+val),multiple=1;
    //             if(v[0][i+1]&v[0][i+2])dp[i+2][1]=max(dp[i+2][1],1+val);
    //             if(v[1][i+1]&v[0][i+1])dp[i+1][2]=max(dp[i+1][2],1+val);
    //         }
    //     }
    // }
    // int tot=max({dp[n][0],dp[n][1],dp[n][2]});
    // if(tot*2==cnt)cout<<(multiple?"Multiple":"Unique")<<endl;
    // else cout<<"None"<<endl;
}
// Unique
// None
