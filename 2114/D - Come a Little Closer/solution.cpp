...
void test_cases(){
    ...
    multiset<int> R(row.begin(),row.end()),C(col.begin(),col.end());
    int ans=1e18;
    f(i,0,n){
        R.erase(R.find(row[i])),C.erase(C.find(col[i]));
        int h=*R.rbegin()-*R.begin()+1;
        int w=*C.rbegin()-*C.begin()+1;
        //cout<<h<<" "<<w<<" "<<endl;
        ans=min(ans,h*w);
        if((h*w)<n)ans+=min(h,w);
        //cout<<ans<<endl;
        R.insert(row[i]),C.insert(col[i]);
    }
    cout<<ans<<endl;
    return;
}
