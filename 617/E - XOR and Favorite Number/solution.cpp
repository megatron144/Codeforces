...
void test_cases(){
    ...
    int blocks=max(1LL,(int)sqrt(n));
    sort(all(nq),[blocks](Query& a,Query& b){
        int a0=a.l/blocks,b0=b.l/blocks;
        if(a0!=b0)return a0<b0;
        return (a0&1)?a.r<b.r:a.r>b.r;
    });
    vi ans(q,0),freq(mx+1,0);
    int curr=0;
    auto add = [&](int val) {
        curr+=freq[val^k];
        freq[val]++;
    ...
    int l=1,r=0;
    for(auto& query : nq) {
        while(l>query.l) add(pre[--l]);
        while(r<query.r) add(pre[++r]);
        while(l<query.l) remove(pre[l++]);
        while(r>query.r) remove(pre[r--]);
        ans[query.id]=curr;
    }
    print(ans);
}
