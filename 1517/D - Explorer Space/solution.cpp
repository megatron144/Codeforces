...
void test_cases(){
    ...
    cin>>n>>m>>k;
    if(k&1){
        f(i,0,n){f(j,0,m)cout<<-1<<" ";cout<<endl;}
        return;
    }
    vector<vi> hor(n,vi(m-1)),ver(n-1,vi(m)),prev(n,vi (m));
    f(i,0,n)f(j,1,m)cin>>hor[i][j-1];
    f(i,1,n)f(j,0,m)cin>>ver[i-1][j];
    f(i,0,k/2){
        vector<vi> curr(n,vi (m,1e10));
        f(i,0,n){
            f(j,0,m){
                int& val=curr[i][j];
                if(j>0)val=min(val,prev[i][j-1]+hor[i][j-1]);
                if(i>0)val=min(val,prev[i-1][j]+ver[i-1][j]);
                if(j+1<m)val=min(val,prev[i][j+1]+hor[i][j]);
                if(i+1<n)val=min(val,prev[i+1][j]+ver[i][j]);
            }
        }
        prev=curr;
    }
    f(i,0,n){f(j,0,m)cout<<prev[i][j]*2<<" ";cout<<endl;}
}
int32_t main() {
