...
void test_cases(){
    ...
    vi set_bit(60,0),v(n);
    take(v);
    for(auto x: v)f(i,0,60)if((x>>i)&1)set_bit[i]++;
    int val=0;
    for(auto x: v){
        int cnt_or=0,cnt_and=0;
        f(j,0,60){
            if((x>>j)&1){
                cnt_or=add_mod(cnt_or,prod_mod((1LL<<j),n));
                cnt_and=add_mod(cnt_and,prod_mod((1LL<<j),set_bit[j]));
            }
            else cnt_or=add_mod(cnt_or,prod_mod((1LL<<j),set_bit[j]));
        }
        val=add_mod(val,prod_mod(cnt_or,cnt_and));
    }
    cout<<val<<endl;
}
int32_t main() {
