...
void test_cases(){
    ...
    // print(sweep);
    int mx=sweep[0];
    f(i,1,n)sweep[i]+=sweep[i-1],mx=max(mx,sweep[i]);
    //print(sweep);
    if(mx==m){
        //cout<<"zero"<<" ";
        int val=1;
    ...
    // print(en);
    f(i,0,n-1){
        if(!st[i+1]){idx=i,zo=0;break;}
        if(!en[i]){idx=i,zo=1;break;}
    }
    //cout<<idx<<endl;
    if(idx!=-1){
        //cout<<"one"<<" "<<idx<<endl;
