...
void test_cases(){
    ...
    int n,m;
    cin>>n>>m;
    vi sweep(n+2),left_most(n+1,n+1),dp(n+1);
    f(i,0,m){
        int l,r;
        cin>>l>>r;
        sweep[l]++,sweep[r+1]--;
        left_most[r]=min(left_most[r],l);
    }
    for(int i=n-1;i>=1;i--)left_most[i]=min(left_most[i],left_most[i+1]);
    int active=0;
    f(i,1,n+1)active+=sweep[i],dp[i]=max(dp[i-1],dp[left_most[i]-1]+active);
    cout<<dp[n]<<endl;
}
int32_t main() {
