...
int limit=500;
int n,m,q,o;
vi state(50005),light_online_neighbors(50005);
vector<vpi> adj(50005);
vector<vi> adj_heavy(50005);
vector<umpi> mp(50005);
void add_edge(int u,int v){
    adj[u].pb({v,+1});
    mp[u][v]=(int)adj[u].size()-1;
    if(adj[u].size()<=limit)
        if(state[u])++light_online_neighbors[v];
    if(adj[u].size()==limit){
        f(i,0,adj[u].size()){
            int w=adj[u][i].first,s=adj[u][i].second;
            if(s>0){
                light_online_neighbors[w]-=state[u];
                adj_heavy[w].pb(u);
            }
            else adj_heavy[w].pb(-u);
        }
    }
    if(adj[u].size()>limit) adj_heavy[v].pb(u);
}
void modify_edge(int u,int v){
    if(mp[u].count(v)==0) return;
    int e=mp[u][v];
    if(e<0||e>=adj[u].size()) return;
    adj[u][e].second*=-1;
    int newsign=adj[u][e].second;
    if(adj[u].size()<limit){
        if(state[u])light_online_neighbors[v]+=(newsign>0?+1:-1);
    }
    else{
        bool found=false;
        f(i,0,(int)adj_heavy[v].size()){
            int &w=adj_heavy[v][i];
            if(w==u){w=-u;found=true;break;}
            if(w==-u){w=u;found=true;break;}
        }
...
void test_cases(){
    ...
    while(q--){
        ...
        if(c=='O'||c=='F'){
            ...
            f(i,0,adj[u].size()){
                int w=adj[u][i].first,s=adj[u][i].second;
                if(s>0)light_online_neighbors[w]+=(state[u]?+1:-1);
            }
        }
        else if(c=='A'){
            cin>>u>>v;
            ...
            cin>>u;
            int ans=light_online_neighbors[u];
            f(i,0,(int)adj_heavy[u].size()){
                int w=adj_heavy[u][i];
                if(w>0)ans+=state[w];
            }
            cout<<ans<<endl;
