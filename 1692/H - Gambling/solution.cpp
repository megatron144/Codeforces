...
void test_cases(){
    ...
    // for(auto x: mp){
    //     cout<<x.first<<" ";
    //     for(auto&[u,v]: x.second)cout<<u<<" "<<v<<" ";
    //     cout<<endl;
    // }
    int mx=1,l=0,r=0,a=v[0];
    for(auto x: mp){
        int pidx=-1,pcnt=0,curr=0;
        int s=-1,e=-1;
        for(auto& [cnt,idx]: x.second){
            if(pidx==-1)curr=cnt,pidx=idx,pcnt=cnt,s=idx,e=idx+cnt-1;
            else{
                int diff=idx-pidx-pcnt;
                if(diff<curr)curr+=cnt-diff;
                else curr=cnt,s=idx;
                pidx=idx,pcnt=cnt,e=idx+cnt-1;
            }
            if(mx<=curr)a=x.first,mx=curr,l=s,r=e;
            //cout<<mx<<" "<<l<<" "<<r<<endl;
        }
    }
    cout<<a<<" "<<l<<" "<<r<<endl;
