...
void test_cases() {
    ...
    // f(i,0,m){
    //     int j = i+1;
    //     while(j<m && v[j]==v[j-1]+1)j++;
    //     int len=j-i;
    //     if(len>=2 && mp[v[i]]>=2 && mp[v[j-1]]>=2){
    //         yn(1);
    //         return;
    //     }
    //     if(len>=3){
    ...
    f(i,0,m) {
        int j=i;
        while (j+1 < m && win[j+1].first==win[j].first+1)++j;
        int cnt=0;
        f(k,i,j+1) {
            if (win[k].second >= 2){
                cnt++;
