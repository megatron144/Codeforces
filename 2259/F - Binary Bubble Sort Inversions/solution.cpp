...
// 2 1 0 0 
// 7 4 2 1 0 0 0 
int query(int i,int j,int cnt0,vi& r,vi& pre){
    if(j>cnt0)return 0;
    if(i>=r[j])return 0;
    int sum_c=pre[r[j]]-pre[i];
    return sum_c-j*(r[j]-i);
}
void test_cases(){
    ...
    vi cnt_val(cnt0+2,0);
    for(auto& x: c)cnt_val[x]++;
    vi r(cnt0+2,0);
    for(int i=cnt0;i>=0;i--)r[i]=r[i+1]+cnt_val[i+1];
    cout<<query(0,0,cnt0,r,pre)<<" ";
    int i=0,j=0;
    f(idx,0,n){
        if(s[idx]=='1')i++;
        else j++;
        cout<<query(i,j,cnt0,r,pre)<<" ";
    }
    cout<<endl;
}
int32_t main() {
