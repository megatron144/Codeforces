...
class SGT{
    ...
    void build(int idx,int l,int h,vi& v){
        ...
        sg_tree[idx]=max(sg_tree[2*idx+1],sg_tree[2*idx+2]);
    }
    int query(int idx,int l,int h,int L,int R){
        if(R<l || L>h)return -1e18;
...
void test_cases(){
    ...
    // print(pre);
    SGT sg1(n),sg2(n);
    sg1.build(0,0,n-1,pre);
    sg2.build(0,0,n-1,suf);
    f(i,0,n){
        int l=sg1.query(0,0,n-1,i+1,nxt[i]-1)-pre[i],r=sg2.query(0,0,n-1,prev[i]+1,i-1)-suf[i];
        // int l=-1e18,r=-1e18;
        // if(i<=nxt[i]-2)l=sg1.query(0,0,n-1,i+1,nxt[i]-1)-pre[i];
        // if(prev[i]<=i-2)r=sg2.query(0,0,n-1,prev[i]+1,i-1)-suf[i];
        if(max(l,r)>0){yn(0);return;}
    }
    yn(1);
}
int32_t main() {
