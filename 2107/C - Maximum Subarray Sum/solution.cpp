...
 
    ...
    umpi suf,pref;
    f(idx,0,seg.size()){
        int l=seg[idx].first, r=seg[idx].second;
        int best=0, curr=0;
        for(int i=r;i>=l;i--){
            curr+=a[i];
            best=max(best,curr);
    ...
    f(j,0,zero.size()){
        ...
        int l=-1,r=-1;
        f(i,0,seg.size()){
            if(seg[i].second==p-1)l=i;
            if(seg[i].first ==p+1)r=i;
        }
        int L = (l==-1?0:suf[l]);
        int R = (r==-1?0:pref[r]);
        int x = k - L - R;
