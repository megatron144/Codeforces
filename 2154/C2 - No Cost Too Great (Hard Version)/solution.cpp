...
void test_cases(){
    ...
    f(i,0,n){
        cin>>b[i];
        if(mn>=b[i])mn=b[i],idx=i;
    }
    map<int,int> mp;
    f(i,0,n){
        for(auto x: pf[a[i]]){
            if(mp[x]){cout<<0<<endl;return;}
            mp[x]++;
        }
    }
    mn=1e10;
    f(i,0,n){
        for(auto x: pf[a[i]])mp[x]--;
        for(auto x: pf[a[i]+1])if(mp[x])mn=min(mn,b[i]);
        for(auto x: pf[a[i]])mp[x]++;
    }
    vi tar;
    f(i,0,n){
        if(i==idx)continue;
        for(auto x :pf[a[i]])tar.pb(x);
    }
    for(auto x: tar){
        if(!(a[idx]%x)){cout<<0<<endl;return;}
        mn=min(mn,(x-(a[idx]%x))*b[idx]);
    }
    sort(all(b));
    mn=min(mn,b[0]+b[1]);
    cout<<mn<<endl;
