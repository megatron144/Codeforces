...
void dfs(int node,int p){
    par[node]=p;
    if(p==-1)odd_eve[node]=0;
    else odd_eve[node]=odd_eve[p]^1;
    for(auto x: adj[node]){
        if(x==p)continue;
        sz[node]++,dfs(x,node);
    }
...
void test_cases(){
    ...
    odd_eve.assign(n+1,0);
    sz.assign(n+1,0);
    par.assign(n+1,-1);
    dfs(n,-1);
    vector<vi> leaves(2);
    f(i,1,n+1)if(!sz[i])leaves[odd_eve[i]].pb(i);
    int st=odd_eve[1];
    vpi ans;
    f(i,1,n){
        if(leaves[st^1].empty()){
            st^=1;
            ans.pb({1,1});
        }
        int node=leaves[st^1].back();
        leaves[st^1].pop_back();
        ans.pb({2,node});
        int p=par[node];
        if(p!=-1){
            sz[p]--;
            if(!sz[p])leaves[odd_eve[p]].pb(p);
        }
        ans.pb({1,1});
        st^=1;
    }
