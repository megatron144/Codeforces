...
void test_cases(){
    ...
    vi freq(mx+1),pre(mx+1);
    for(auto x:v)freq[x]++;
    f(i,1,mx+1)pre[i]=pre[i-1]+freq[i];
    int ans=-1e18;
    f(i,2,mx+2){
        int price_mx=(mx+i-1)/i;
        int total=0,reuse=0;
        f(j,1,price_mx+1){
            int l=(j-1)*i+1,r=min(j*i,mx);
            if(l>r)continue;
            int cnt=pre[r]-pre[l-1];
            if(!cnt)continue;
            total+=j*cnt;
            if(j<=mx)reuse+=min(freq[j],cnt);
        }
        ans=max(ans,total-cost*(n-reuse));
    }
    cout<<ans<<endl;
}
int32_t main() {
