...
void test_cases(){
    ...
    take(b);
    vi vis(n+1,0);
    vector<vi> val(n,vi(2,0));
    f(i,0,n)val[i][0]=a[i],val[i][1]=b[i];
    int skip=1,swap=1;
    f(i,0,n-1){
        int skip_new=0,swap_new=0;
        f(j,0,2){
            int curr=(j==0?skip:swap);
            f(k,0,2){
                if(val[i][j]<=val[i+1][k] && val[i][1-j]<=val[i+1][1-k]){
                    if(k==0)skip_new=add_mod(skip_new,curr,MOD);
                    else swap_new=add_mod(swap_new,curr,MOD);
                }
            }
        }
        skip=skip_new;
    ...
    cout<<add_mod(skip,swap,MOD)<<endl;
}
int32_t main() {
