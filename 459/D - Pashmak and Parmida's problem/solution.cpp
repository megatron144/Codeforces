...
void test_cases(){
    ...
    vi pre,post(n);
    umpi mp;
    for(auto x: v)mp[x]++,pre.pb(mp[x]),mx=max(mx,mp[x]);
    mp.clear();
    for(int i=n-1;i>=0;i--){
        post[i]=++mp[v[i]];
    }
    ...
    // print(pre);
    // print(post);
    vi freq_of_freq(mx+1);
    for(auto x: post)freq_of_freq[x]++;
    //print(freq_of_freq);
    SGT sg(mx+1);
    sg.build(0,0,mx,freq_of_freq);
    f(i,0,n){
        sg.update(0,0,mx,post[i],-1);
        if(pre[i]>1)cnt+=sg.query(0,0,mx,0,pre[i]-1);
    }
    cout<<cnt<<endl;
}
int32_t main() {
