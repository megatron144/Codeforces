...
vector<vpi> v;
set<int> vis;
void src_dfs(int p,int node,int val,int& dst,bool& done){
    if(node==dst){
        if(!val)done=true;
        return;
    }
    vis.insert(val);
    for(auto&[u,w]: v[node])if(!(u==p))src_dfs(node,u,w^val,dst,done);
}
bool dst_dfs(int p,int node,int val,int& dst){
    if(node!=dst && vis.count(val))return true;
    for(auto&[u,w]: v[node])
        if(!(u==p) && dst_dfs(node,u,w^val,dst))return true;
    return false;
}
void test_cases(){
    ...
    v.assign(n+1,{});
    int u,vv,w;
    f(i,1,n)cin>>u>>vv>>w,v[u].pb({vv,w}),v[vv].pb({u,w});
    src_dfs(0,s,0,d,done);
    if(done){yn(1);vis.clear();return;}
    yn(dst_dfs(0,d,0,d));
    vis.clear();
}
int32_t main() {
