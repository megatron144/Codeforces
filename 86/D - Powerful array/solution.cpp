...
void test_cases(){
    ...
    sort(all(query), [blocks](const vector<int>& a, const vector<int>& b) {
        if (a[0]/blocks != b[0]/blocks) return a[0]/blocks<b[0]/blocks;
        return ((a[0]/blocks)&1)?(a[1] > b[1]) : (a[1] < b[1]);
    });
    int curr=0;
    vi ans(q,0),freq(*max_element(all(v))+1,0);
    auto addE=[&](int idx) {
        auto& cnt=freq[v[idx]];
        curr+=(v[idx]*(2*cnt+1));
        cnt++;
