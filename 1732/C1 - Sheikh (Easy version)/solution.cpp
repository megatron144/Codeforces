...
void test_cases(){
    ...
    vi v(n);
    take(v);
    vi psum(n+1,0),pxor(n+1,0);
    f(i,0,n)psum[i+1]+=psum[i]+v[i],pxor[i+1]^=pxor[i]^v[i];
	while(q--){
	    int l,r;
	    cin>>l>>r;
	    l--;
	    int ans=psum[r]-psum[l]-(pxor[r]^pxor[l]);
	    int lo=0,hi=r-l,x=l+1,y=r,mid;
	    while(lo+1<hi){
	        mid=lo+((hi-lo)>>1);
	        int tmp_sum=psum[l+mid]-psum[l],tmp_xor=pxor[l+mid]^pxor[l];
	        for(int i=l;i+mid<=r;i++){
	            if((tmp_sum-tmp_xor)==ans){
	                hi=mid;
	                x=i+1,y=mid+i;
	                goto there;
	            }
	            tmp_xor^=v[i],tmp_sum-=v[i];
	            if(i+mid<r)tmp_sum+=v[i+mid],tmp_xor^=v[i+mid];
	        }
	        lo=mid;
	        there:
	    }
