...
void test_cases(){
    ...
    vi v(n);
    take(v);
    vector<vi> prev(n+1,vi(n+1,0));
    prev[0][0]=1;
    f(i,0,n){
        vector<vi> curr(n+1,vi(n+1,0));
        f(j,0,n+1){
            f(k,0,n+1){
                if(!prev[j][k])continue;
                curr[j][k]=add_mod(curr[j][k],prev[j][k]);
                if(j<=v[i])curr[v[i]][k]=add_mod(curr[v[i]][k],prev[j][k]);
                else if(k<=v[i])curr[j][v[i]]=add_mod(curr[j][v[i]],prev[j][k]);
            }
        }
        swap(curr,prev);
    }
    int ans=0;
    f(i,0,n+1)f(j,0,n+1)ans=add_mod(ans,prev[i][j],MOD);
    cout<<ans<<endl;
}
// 13
// 73
