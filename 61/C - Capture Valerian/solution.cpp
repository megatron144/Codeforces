...
void test_cases(){
    ...
    int prod=1;
    f(i,0,c.size()){
        if(c[i]<='9' && c[i]>='0')tmp+=(int)(c[i]-'0')*prod;
        else tmp+=((int)(c[i]-'A')+10)*prod;
        prod*=a;
    }
    //cout<<tmp<<endl;
    if(b=="R"){
        ...
        tmp=min(tmp,3000LL);
        while(tmp > 0){
            if(tmp >= 1000){ cout << string(tmp/1000,'M'); tmp %= 1000; continue; }
            if(tmp >= 900){ cout << "CM"; tmp -= 900; continue; }
            if(tmp >= 500){ cout << "D"; tmp -= 500; continue; }
            if(tmp >= 400){ cout << "CD"; tmp -= 400; continue; }
            if(tmp >= 100){ cout << "C"; tmp -= 100; continue; }
            if(tmp >= 90){ cout << "XC"; tmp -= 90; continue; }
            if(tmp >= 50){ cout << "L"; tmp -= 50; continue; }
            if(tmp >= 40){ cout << "XL"; tmp -= 40; continue; }
            if(tmp >= 10){ cout << "X"; tmp -= 10; continue; }
            if(tmp == 9){ cout << "IX"; tmp -= 9; continue; }
            if(tmp >= 5){ cout << "V"; tmp -= 5; continue; }
            if(tmp == 4){ cout << "IV"; tmp -= 4; continue; }
            cout << "I"; tmp -= 1;
        }
        return;
    }
    ...
    int curr=1;
    int nb=stoi(b);
    while(curr<tmp)curr*=nb;
    int can=0;
    while(curr){
        int q=tmp/curr;
        if(q>=10)can=1,cout<<(char)('A'+(q-10));
        else if(q | can)can=1,cout<<q;
        tmp-=(q*curr);
        curr/=nb;
    }
    return;
