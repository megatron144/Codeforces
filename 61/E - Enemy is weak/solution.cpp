...
void test_cases(){
    ...
    mx=0;
    for(auto& x: mp)x.second=++mx;
    for(auto& x: v)x=mp[x];
    //print(v);
    vi f1(mx+1,0),f2(mx+1,0);
    for(auto x: v)f2[x]++;
    SGT sg1(mx+1),sg2(mx+1);
    sg1.build(0,0,mx,f1);
    sg2.build(0,0,mx,f2);
    int cnt=0;
    for(auto x: v){
        f2[x]--;
        sg2.update(0,0,mx,x,f2[x]);
        int big=sg1.query(0,0,mx,x+1,mx);
        int smol=sg2.query(0,0,mx,0,x-1);
        f1[x]++;
        sg1.update(0,0,mx,x,f1[x]);
        cnt+=smol*big;
    }
    cout<<cnt<<endl;
}
