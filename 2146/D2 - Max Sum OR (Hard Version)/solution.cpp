...
vi v;
void generate(int l,int r,int j,int left){
    if(l>r)return;
    if(l==r){
        v[l-left]=r;
        return;
    ...
    int mid=l;
    while(mid+1<=r && ((mid+1)>>j)==(l>>j))mid++;
    if(mid==r){
        generate(l,r,j-1,left);
        return;
    }
    int lower=mid+1,upper=mid;
    while(lower-1>=l && upper+1<=r){
        v[--lower-left]=++upper;
        v[upper-left]=lower;
    }
    generate(l,lower-1,j-1,left);
    generate(upper+1,r,j-1,left);
}
void test_cases(){
    int l,r;
    cin>>l>>r;
    ...
    generate(l,r,30,l);
    int val=0;
    f(i,l,r+1)val+=(v[i-l]|i);
    cout<<val<<endl;
    print(v);
    v.clear();
}
// 12 90 240 70 20 28 3000000039
int32_t main() {
