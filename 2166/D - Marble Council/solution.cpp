...
void test_cases(){
    ...
    vi frq(n+1,0),c;
    for(auto x: v)frq[x]++;
    f(i,1,n+1)if(frq[i])c.pb(frq[i]);
    if(!c.size()){cout<<0<<endl;return;}
    sort(all(c));
    int sz=c.size(),tot=0;
    for(auto x: c)tot+=x;
    vi pre(sz+1,0),suf(sz+1,1);
    f(i,0,sz)pre[i+1]=pre[i]+c[i];
    for(int i=sz-1;i>=0;i--)suf[i]=prod_mod(suf[i+1],c[i]);
    vi dp(tot+10,0);
    dp[0]=1;
    int idx=0,ans=0;
    f(i,1,tot+1){
        while(idx<sz && c[idx]<=i){
            int w=c[idx];
            for(int j=tot;j>=w;j--)
                if(dp[j-w])dp[j]=add_mod(dp[j],prod_mod(dp[j-w],w));
            idx++;
        }
        int tar=tot-pre[idx];
        if(tar>i)continue;
        ans=add_mod(ans,prod_mod(suf[idx],dp[i-tar]));
    }
    cout<<ans<<endl;
}
// 7
