...
public:
    int n, LOG;
    vector<vi> st;
    SparseTable_MX(int n, int LOG=20):n(n),LOG(LOG) {
...
public:
    int n, LOG;
    vector<vi> st;
    SparseTable_MN(int n, int LOG=20):n(n),LOG(LOG) {
        st.assign(n,vi(LOG, 0));
    }
    void build(vi& v) {
        f(i,0,n)st[i][0]=v[i];
...
void test_cases(){
    ...
    f(i,1,n+1)cin>>a[i];
    f(i,1,n+1)cin>>b[i];
    int d=k%2,ans=a[1],curr=a[1];
    f(i,2,n+1){
        curr=max(a[i],curr+a[i]);
        ans=max(curr,ans);
    }
    if(!d){cout<<ans<<endl;return;}
    ...
    ans=-1e18;
    f(i,1,n+1){
        int mini=mn.query(l[i], i-1),maxi=mx.query(i,r[i]-1);
        if(mini==1e16 || maxi==-1e16) continue;
        ans = max(ans,maxi-mini+b[i]);
    }
    if(ans==-1e18)ans=tmp;
    cout<<ans<<endl;
}
