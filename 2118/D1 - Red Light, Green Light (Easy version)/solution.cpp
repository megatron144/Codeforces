...
void test_cases(){
    ...
    while(q--){
        int loc;
        cin>>loc;
        if(loc>pos[n-1]){yn(1);continue;}
        vector<vi> vis(n,vi (2,0));
        int idx=lower_bound(all(pos),loc)-pos.begin();
        int dist=pos[idx]-loc;
        int dir=1;
        while(1){
            if(dist>=delay[idx] && !((dist-delay[idx])%k)){
                int j=1;
                if(dir==-1)j--;
                if(vis[idx][j]){yn(0);goto there;}
                vis[idx][j]=1;
                dir*=-1;
                
            }
            idx+=dir;
            if(idx==-1 || idx==n){yn(1);goto there;}
            dist+=abs(pos[idx]-pos[idx-dir]);
        }
        there:
    }
}
