...
int n,inv_cnt;
vector<vi> p,dp;
int func(int used,int sum){
    if(used==n) return (sum==inv_cnt);
    if(dp[used][sum]!=-1)return dp[used][sum];
    for(int s=1;s<=n-used;s++){
        int add=s*(s-1)/2;
        if(sum+add>inv_cnt) break;
        if(func(used+s,sum+add)){
            p[used][sum]=s;
            return dp[used][sum]=1;
        }
    }
    return dp[used][sum]=0;
}
void test_cases(){
    ...
    int total=n*(n-1)/2;
    inv_cnt=total-k;
    dp.assign(n+1,vi (inv_cnt+1,-1));
    p.assign(n+1,vi(inv_cnt+1,-1));
    if(func(0,0)==0){cout<<0<<endl;return;}
    vi order;
    int used=0,sum = 0;
    while(used<n){
        int val=p[used][sum];
        if(val<=0){cout<<0<<endl; return;}
        order.pb(val);
        used+=val;
        sum+=val*(val-1)/2;
    }
