...
void test_cases(){
    ...
    //cout<<prod_mod(255,power(8,MOD-2,MOD))<<endl;
    int n,m,k,tot=0;
    cin>>n>>m>>k;
    int u,v,w;
    f(i,0,m){
    ...
    f(i,0,k){
        int sum=((i*(i+1))/2)%MOD; 
        int prb=prod_mod(ncr(k,i+1),power(den_inv,i+1,MOD));
        int skip=prod_mod(den-1,den_inv);
        prb=prod_mod(prb,power(skip,k-i-1,MOD));
        inc=add_mod(inc,prod_mod(sum,prb));
    }
    cout<<add_mod(prod_mod(prod_mod(tot,k),den_inv),prod_mod(m,inc))<<endl;
}
// 0
// 55
// 777777784
