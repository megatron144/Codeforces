...
void test_cases(){
    ...
    int cnt = 0;
    const long long MAX_VAL_LL = 1000000000LL; 
    const long long K_LL = 1000000LL;  
    for(auto pr : mp){
        int num = pr.first;
        int val = pr.second;
        if ((long long)num > K_LL ){
            for(long long b = 2; b * (long long)num <= MAX_VAL_LL; ++b){
                if(num % b == 0){
                    int left = num / (int)b;
                    long long right_ll = b * (long long)num; 
                    if(right_ll > MAX_VAL_LL) break;
                    int right = (int) right_ll;
                    if(mp.count(left) && mp.count(right)){
                        cnt += (long long)mp[left]*mp[right]*val;
                    }
                }
            }
        } else {
            for(long long r = 2; r * r <= (long long)num; ++r){
                if(num % r == 0){
                    int left = num / (int)r;
                    long long right_ll = (long long)num*r; 
                    if(right_ll <= MAX_VAL_LL && mp.count(left) && mp.count((int)right_ll)){
                        int right = (int)right_ll;
                        cnt += (long long)mp[left] * mp[right] * val;
                    }
                    if(r * r != (long long)num){
                        long long lim_ll = (long long)num * (long long)num / r;
                        if(lim_ll <= MAX_VAL_LL && mp.count((int)r) && mp.count((int)lim_ll)){
                            cnt += (long long)mp[(int)r] * mp[(int)lim_ll] * val;
                        }
                    }
                }
            }
            if(num > 1){
                ...
                if(sq <= MAX_VAL_LL && mp.count(1) && mp.count((int)sq)){
                    cnt += (long long)mp[1] * mp[(int)sq] * val;
                }
            }
        }
    }
    for(auto x : v){
        if(mp[x] >= 3) cnt += (long long)(mp[x]-1) * (mp[x]-2);
    }
    cout<<cnt<<endl;
}
 
int32_t main() {
    ...
    while(t--)test_cases();
    clock_t end = clock(); 
    auto elapsed = (end - start) / (1.0L * CLOCKS_PER_SEC);
    cerr << "Total elapsed time: " << fixed << setprecision(6) << elapsed << " seconds
";
    return 0;
}