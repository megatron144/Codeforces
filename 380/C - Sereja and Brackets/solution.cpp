...
void build(int n,vi& v){
    ...
    for(int k=1;k<LOG;k++){
        for(int i=0;i+(1<<k)<=n;i++){  
            sparse_table[i][k]=max(sparse_table[i][k-1],
...
struct Node {
    ...
    int close;
    int balanced;
    Node():open(0),close(0),balanced(0){}
    Node(int o,int c,int b):open(o),close(c),balanced(b){}
};
class SGT{
    public:
    vector<Node> sg_tree;
    SGT(int n){
        //open,close,balanced
        sg_tree.resize(4*n+1,Node());
    }
    Node merge(Node& a,Node& b){
        int mini=min(a.open,b.close);
        int balanced_new=a.balanced+b.balanced+mini;
        int open_new=a.open+b.open-mini;
        int close_new=a.close+b.close-mini;
        return Node(open_new,close_new,balanced_new);
    }
    void build_sg(int idx,int l,int h,string& s){
        if(l==h){
            sg_tree[idx]=Node(s[l]=='(',s[l]==')',0);
            return;
        }
        int mid=l+((h-l)>>1);
        build_sg(idx*2+1,l,mid,s);
        build_sg(idx*2+2,mid+1,h,s);
        sg_tree[idx]=merge(sg_tree[2*idx+1],sg_tree[2*idx+2]);
    }
    Node query_sg(int idx,int l,int h,int L,int R){
        if(R<l || L>h)return Node();
        if(l>=L && h<=R)return sg_tree[idx];
        int mid=l+((h-l)>>1);
        Node left=query_sg(2*idx+1,l,mid,L,R);
        Node right=query_sg(2*idx+2,mid+1,h,L,R);
        return merge(left,right);
    }
    // void update_sg(int idx,int l,int h,int pos,int val){
    //     if(l==h){
    //         sg_tree[idx]=val;
    //         return;
    //     }
    //     int mid=l+((h-l)>>1);
    //     if(pos<=mid)update_sg(idx,l,mid,pos,val);
    //     else update_sg(idx,mid+1,h,pos,val);
...
void test_cases(){
    ...
    n=s.size();
    SGT sg(n);
    sg.build_sg(0,0,n-1,s);
    int q;
    cin>>q;
    while(q--){
        int l,r;
        cin>>l>>r;
        cout<<sg.query_sg(0,0,n-1,l-1,r-1).balanced*2<<endl;
    }
}
int32_t main() {
