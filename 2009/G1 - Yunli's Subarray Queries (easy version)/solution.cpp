...
void test_cases(){
    ...
    take(v);
    map<int,int> mp;
    multiset<int> max_freq;
    vi ans(n-k+1);
    f(i,0,k){
        int key=v[i]-i;
        if(mp.count(key))max_freq.erase(max_freq.find(mp[key]));
        mp[key]++;
        max_freq.insert(mp[key]);
    }
    ans[0]=k-*max_freq.rbegin();
    f(i,k,n){
        int addKey=v[i]-i;
        if(mp.count(addKey))max_freq.erase(max_freq.find(mp[addKey]));
        mp[addKey]++;
        max_freq.insert(mp[addKey]);
        int remKey=v[i-k]-(i-k);
        max_freq.erase(max_freq.find(mp[remKey]));
        mp[remKey]--;
        if(mp[remKey]==0)mp.erase(remKey);
        else max_freq.insert(mp[remKey]);
        ans[i-k+1]=k-*max_freq.rbegin();
    }
    while(q--){
        int l,r;
        cin>>l>>r;
