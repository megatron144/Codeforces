...
void test_cases(){
    ...
    map<int,pii> mp;
    f(i,0,n){
        if(mp.find(r[i])==mp.end())mp[r[i]]={c[i],c[i]};
        else{
            mp[r[i]].first=min(mp[r[i]].first,c[i]);
            mp[r[i]].second=max(mp[r[i]].second,c[i]);
        }
    }
    int prevx=x,pl=y,pr=y;
    int dp0=0,dp1=0;
    for(auto &it: mp){
        int X=it.first,l=it.second.first,r=it.second.second;
        int dx=X-prevx;
        int len=r-l;
        int nd0=1e18,nd1=1e18;
        nd0=min(nd0,dp0+dx+len+min(abs(pl-l)+abs(l-r),abs(pl-r)));
        nd0=min(nd0,dp1+dx+len+min(abs(pr-l)+abs(l-r),abs(pr-r)));
        nd1=min(nd1,dp0+dx+len+min(abs(pl-l),abs(pl-r)+abs(r-l)));
        nd1=min(nd1,dp1+dx+len+min(abs(pr-l),abs(pr-r)+abs(r-l)));
        dp0=nd0;
        dp1=nd1;
        prevx=X;
        pl=l;
    ...
    int ans=min(dp0+a-prevx+abs(pl-b),dp1+a-prevx+abs(pr-b));
    cout<<ans<<endl;
}
int32_t main() {
