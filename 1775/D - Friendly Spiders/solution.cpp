...
void test_cases(){
    ...
    take(v);
    int mx=0;
    vi mark(300005,-1),marked(300005);
    f(i,0,n){
        mx=max(v[i],mx);
        mark[v[i]]=i;
    }
    vi vis(mx+5,0),par(mx+5,-1);
    int l,r;
    cin>>l>>r;
    if(l==r){
        cout<<1<<endl<<l<<endl;
    ...
    if(gcd_stein(v[--l],v[--r])!=1){
        cout<<2<<endl<<l+1<<" "<<r+1<<endl;
        return;
    }
    queue<int> q;
    int val=v[l];
    ...
    while(!q.empty()){
        ...
        if(v[r]%p==0){
            ...
            ans.pb(r+1);
            while(curr!=-1){
                ans.pb(marked[curr]+1);
                curr=par[curr];
            }
            int sz=ans.size();
            cout<<sz<<endl;
            f(i,0,sz)cout<<ans[sz-i-1]<<" ";
            cout<<endl;
            return;
        }
        for(int j=p;j<=mx;j+=p){
            if(mark[j]!=-1 && !vis[j/p] && !vis[j]){
                int val=j;
                while(val!=1){
                    int key=sp[val];
                    if(!vis[key]){
